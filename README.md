
While most sophomore are doing homework, I'm shipping AI systems to real clients.  
Founder @ [Beconix LLC](https://www.beconix.com/home) · CS @ Hunter College CUNY

---

### 🔨 What I'm Building
- Voice AI agents that book, reschedule, and report — without a human touching anything
- Automation pipelines running 24/7 for a $1M+ revenue business
- A dispatch platform that manages field service jobs autonomously

---

### 🧰 Tools
`Python` `C++` `SQL` `AWS` `n8n` `Docker` `OpenAI API` `REACT` `LLMS` `Tableau`  
*Learning:* `PyTorch` · `LangGraph` · `Hugging Face` 

---

### 📫 Let's Talk
- 📧 bilalzahid132@gmail.com
- 💼 [LinkedIn](https://www.linkedin.com/in/muhammad-bilal-beconix/)  
- 🌐 [beconix.com](https://www.beconix.com/home)
